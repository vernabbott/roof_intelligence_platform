...
def analysis_prompt(row: dict) -> str:
    ...
    return (
        ...
        "Customer-facing observations, summary, and recommendation must discuss only the roof, its visible condition, "
        "risk, and appropriate next steps. Never mention AI, workflow stages, reviewer confirmation, reference images or "
        "filenames, matching logic, similarity scores, parcel matching, or how the material was identified in those fields. "
        ...
        "On ribbed metal roofs, repeated narrow rectangular panels that share a consistent size, orientation, spacing, "
        "and alignment with the metal ribs may be translucent fiberglass skylight panels even when their curbs are not "
        "prominent. Distinguish that organized array from isolated repair patches, coating wear, shadows, and image artifacts. "
        "Report supported panels as skylights present. If recommending silicone restoration, state that their light-transmitting "
        "faces must be masked, protected, and excluded from coating so they do not become opaque or difficult to identify. "
        ...
        "Score roof condition using visible serviceability risks, not only apparent membrane age. Within one continuous "
        "roof field, uneven color, mottling, stains, and light-to-dark weathering bands are condition variation rather "
        "than proof of another roof material. Consider aging, UV weathering, chalking, dirt, prior repairs, moisture "
        "retention, and drainage patterns. Mark suspected ponding only when retained water, basin-shaped staining, "
        "sediment rings, algae-like growth, or drainage concentration supports it. Do not infer leakage or wet insulation "
        "from discoloration alone. For every all-metal roof, set suspected_ponding to false and omit ponding, standing-water, "
        "and retained-water statements from observations and risk notes. Metal panel roofs are treated as shedding water by "
        "design; weathering, coating wear, replacement panels, repairs, shadows, and tonal variation are not ponding evidence. Reduce the applicable "
        "condition scores in proportion to the visible evidence. Numerous penetrations, skylights, vents, curbs, or rooftop units "
...
def roof_candidate_prompt(row: dict, config: RoofReferenceConfig) -> str:
    ...
    return (
        ...
        "metadata. Divide materially different roof areas into zones before choosing candidates. A material-zone boundary "
        "requires a parapet, ridge, expansion joint, elevation or slope change, addition boundary, or non-color evidence "
        "of different construction. Do not split one continuous field into multiple material zones from color variation, "
        "mottling, aging, weathering, stains, repairs, or possible ponding alone. For every zone, first "
        ...
        "evidence is visible. When an aged light roof remains within the white single-ply/coating family but weathering "
        "obscures the details needed to separate TPO, PVC, and coating, keep one zone and carry all supported candidates "
        "forward for the controlled tpo_pvc_or_coating result. Do not apply a TPO default to weathered asphaltic or aggregate-textured roofs. "
        "Treat no_visible_seams as an observation only when the image is sharp enough that seams should be visible; otherwise "
        "record uncertain. Light color and apparent smoothness alone do not establish white single-ply. When an adequately "
        "visible light or off-white field has no broad repeated sheet-seam grid, welded details, or membrane flashing construction "
        "and instead appears muted, monolithic, asphaltic, weathered, or coating-like, rank modified bitumen and coating before "
        "TPO/PVC; carry mod_bit and coating forward when they cannot be separated. Conversely, a bright white finish does "
        "not establish single-ply when dense fine straight raised ribs remain uniformly spaced and continuous across pitched "
        "rigid planes. Classify metal only when both manufactured narrow-rib evidence and visible rigid slope geometry are "
        "present, normally a peak or raised ridge with ribs running perpendicular downslope. Broad sheet seams, attachment "
        "rows, or image-similarity to a metal example cannot substitute for either requirement. A continuous flexible-looking flat field with low-profile roll/lap banding, "
        "localized overlapping patches, and no raised-rib shadows, rigid drainage planes, directional sheen, or metal "
        "ridge/edge construction favors modified bitumen. Do not split that continuous membrane into multiple metal zones "
        "from weathering bands. A continuous raised center ridge with dense narrow ribs terminating perpendicular to it on "
        "opposing shallow planes strongly favors metal, even when the overhead view makes the roof look nearly flat. "
        "Contrasting rectangles that occupy consistent panel-bay widths and align precisely with that rib grid support "
        "replaced metal panel sections rather than irregular membrane patches. Use the canonical key metal without assigning "
        "a metal subtype. Use estimated_area_percentage=0 when the visible share cannot "
...
def reference_analysis_prompt(
    ...
    return (
        ...
        "ponding only with affirmative visible evidence such as standing or retained water, a basin-shaped wet pattern, "
        "sediment or tide rings, algae-like concentration, or a blocked-drain pattern; discoloration and streaking alone "
        "are insufficient, and do not infer leaks or wet insulation from color alone. For every all-metal roof, set "
        "suspected_ponding to false and omit ponding, standing-water, and retained-water statements from observations and "
        "risk notes. Metal panel roofs are treated as shedding water by design. Heavy weathering may lower material confidence; use tpo_pvc_or_coating for one aged light-colored "
        "zone when TPO, PVC, and coating remain unresolved. Use estimated_area_percentage=0 if an area share cannot "
        ...
        "Do not use the white single-ply family from brightness and smoothness alone. For an adequately visible light or "
        "off-white monolithic field without a broad repeated membrane-sheet layout, welded details, or thermoplastic flashing "
        "construction, compare modified bitumen and coating first and use mod_bit_or_coating when those remain unresolved. "
        "Do not apply this rule when blur, obstruction, glare, or compression prevents a reliable seam observation. "
        "Use metal as the type without a standing-seam, ribbed, or corrugated subtype. Metal is gated by two required target-image findings: "
        "(1) dense, narrow, uniformly repeated manufactured raised ribs and (2) rigid pitched geometry, normally a visible peak or raised ridge "
        "with those ribs running perpendicular downslope. If either finding is absent or unresolved, do not return metal, even when a metal "
        "reference has the highest similarity score. Broad rectangular sheet seams or attachment rows on a smooth flat field are membrane "
        "evidence, not metal ribs. A dark, flat, matte attached roof without the required metal geometry should favor EPDM over metal. "
        "A continuous raised center ridge plus dense narrow ribs terminating perpendicular to it on opposing shallow roof "
        "planes is strong metal evidence even when pitch is subtle. Clearly resolved exposed eaves or rakes, crisp "
        "shedding-edge trim, gutters, or downspouts reinforce metal when they agree with that ridge, pitch, and downslope "
        "rib geometry. Apparent parapet absence alone is not a material test, and parapet presence or absence must remain "
        "out of customer-facing roof-system conclusions. Uniform contrasting rectangles that occupy complete "
        "panel-bay widths and align with the rib grid indicate replaced metal panel sections and prior repair activity; do "
        "not reinterpret the surrounding rigid ribbed assembly as modified bitumen or coating. "
        "The same dense continuous manufactured rib geometry strongly favors metal when the finish is bright white; do not "
        "default a white ribbed rigid field to TPO/PVC or coating from color. "
        "A sharp transition between two large color fields does not create separate materials when a peak ridge, opposing "
        "pitched planes, perpendicular rib direction, rib spacing, and rigid metal-panel geometry continue through both "
        "tones. Keep the roof metal and record installation date, panel replacement, coating age, finish, or weathering as "
        "unresolved possible explanations unless closer evidence establishes the cause. Localized contrasting patches concentrated "
        "around metal penetrations, curbs, seams, or fasteners indicate prior repair activity and elevated flashing/seal "
        "leakage risk, but do not prove an active leak. Preserve the metal substrate when its ribs remain visible. A continuous "
        "repairable-looking metal field may be recommended for conditional silicone-restoration evaluation, subject to onsite "
        "inspection, moisture and adhesion testing, dry sound panels and insulation, substrate compatibility, required repairs "
        "and preparation, drainage review, and manufacturer-system and warranty eligibility. On a metal roof, repeated "
        "same-size rectangular panels aligned with and interrupting the rib pattern in an evenly spaced array may be translucent "
        "fiberglass skylight panels rather than patches. When supported, set skylights to present and exclude the panel faces "
        "from any silicone coating recommendation; require masking and protection to preserve light transmission and visibility. "
        "Irregular weathering streaks, coating loss, runoff marks, repair edges, membrane seams, or application marks that vary in width, "
        "spacing, continuity, or direction are not metal ribs. Without both repeated narrow raised-rib evidence and rigid pitched/ridged "
        "geometry, do not classify the roof as metal. A visibly flat smooth white field with broad low-profile seams belongs in the "
        "white single-ply/coating family. "
        "A continuous flexible-looking flat field with low-profile roll/lap banding, localized overlapping repair patches, "
        "and none of the rigid metal cues favors modified bitumen. Do not split one continuous membrane into multiple metal "
        "zones from broad weathering bands. On an aged gray-black asphaltic field, small contrasting rectangles with local "
        "overlap edges or interruption of the roll pattern may be repair patches; distinguish them from curbs, equipment, "
        "shadows, and image artifacts. Irregular soft-edged tan basin patterns may support recurring ponding when drainage "
        "context is visible and they differ from straight-edged patches or material boundaries. Widespread aging plus repeated "
        "repairs and affirmative ponding evidence may support probable end-of-service-life condition and elevated leakage "
        "risk, but never claim an active leak, wet insulation, or exact remaining life without onsite verification. "
        "Modified-bitumen roll laps are low-profile sheet seams and do not normally create a full-field raised-rib pattern "
        "across multiple sloped planes. Treat repair bands and coating wear as condition evidence rather than proof of an "
        "asphaltic substrate. Determine roof color from the continuous exposed field, not from bright seam tape, repair "
        "bands, curbs, rooftop equipment, glare, or compression highlights. A dark charcoal field is not white single-ply "
        "because localized linear or rectangular details appear white; the field itself must be predominantly white, cream, "
        "or very light gray before using the white single-ply family. Broad low-profile rectangular seams on a smooth dark "
        "field favor EPDM, while narrow frequent roll laps, granules, and asphaltic texture favor modified bitumen. "
        "For a dark membrane zone "
        ...
        "modified bitumen and coating, use mod_bit_or_coating. When a tan weathered low-slope field could be modified "
        "bitumen, a coated roof, or tar-and-gravel/BUR because neither roll laps, coating application evidence, nor "
        "embedded aggregate is resolved, use mod_bit_coating_or_tar_and_gravel and retain all three specific keys as "
        "alternatives. Never return standalone pvc or coating as the final type; "
